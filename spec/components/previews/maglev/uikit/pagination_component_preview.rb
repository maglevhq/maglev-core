...
class Maglev::Uikit::PaginationComponentPreview < ViewComponent::Preview
  include Pagy::Backend

  # @param hidden_if_single_page toggle
  def none(hidden_if_single_page: false)
    pagy, _ = pagy_array([], limit: 5)
    render(Maglev::Uikit::PaginationComponent.new(pagy:, hidden_if_single_page:, item_name: 'account'))
  end

  # @param hidden_if_single_page toggle
  def one(hidden_if_single_page: false)
    pagy, _ = pagy_array(accounts.take(1), limit: 5)
    render(Maglev::Uikit::PaginationComponent.new(pagy:, hidden_if_single_page:, item_name: 'account'))
  end

  # @param hidden_if_single_page toggle
  def many(hidden_if_single_page: false)
    pagy, _ = pagy_array(accounts, limit: 5)
    render(Maglev::Uikit::PaginationComponent.new(pagy:, hidden_if_single_page:, item_name: 'account'))
  end
