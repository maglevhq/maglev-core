...
RSpec.describe 'Maglev::Admin::Sections::PreviewsController', type: :request do
  ...
  context 'previewing a section' do
    ...
    it 'renders the HTML of a section within the theme layout' do
      ...
      expect(response.body).to match(%r{<h1 data-maglev-id="[0-9a-zA-Z\-_.]+\.title" class="[^>]+">Title</h1>})
      expect(response.body).to match(%r{<div data-maglev-id="[0-9a-zA-Z\-_.]+\.body" class="[^>]+">Body</div>})
    end
  end
end
