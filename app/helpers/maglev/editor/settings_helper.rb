  ...
  module Editor
    module SettingsHelper
      ...
      def section_settings_paths
        {
          collection_items_path: ->(definition, _) { editor_combobox_collection_items_path(collection_id: definition.options[:collection_id]) },
          assets_path: ->(_, context) { editor_assets_path(source: context[:source], picker: true) },          
          edit_link_path: ->(_, context) { edit_editor_link_path(link: context[:value], input_name: context[:source]) },
