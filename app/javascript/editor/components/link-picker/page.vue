  ...
  watch: {
    currentLink: {
      ...
      async handler(currentLink) {
        if (!currentLink || currentLink.linkType !== 'page' || this.isBlank(currentLink.linkId) || currentLink.linkId === this.page?.id) return
        this.page = await this.services.page.findById(this.currentSite, currentLink.linkId)
        if (this.page)
          this.pageSection = this.page.sectionNames.find(item => item.id === currentLink.sectionId)        
      }
    },
    page(page) {
      this.$emit('change', {
