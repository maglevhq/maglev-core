<template>
  <div class="col-span-1" @mouseover="hovered = true" @mouseleave="hovered = false">
    ...
    <div class="px-2 mb-1 flex items-center text-xs justify-between text-gray-700">
      <span :class="{ 'invisible': !hasDimensions }">{{ image.width }}x{{ image.height }}</span>
      <span :class="{ 'invisible': !hasSize }">{{ image.byteSize | numberToHumanSize }}</span>
    </div>
  </div>
</template>
