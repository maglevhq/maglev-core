<template>  
  <div class="flex items-center py-3 pl-6 hover:bg-editor-primary hover:bg-opacity-5 transition-colors duration-200">
    <router-link :to="{ name: 'editPage', params: { pageId: page.path } }" class="flex flex-grow items-center text-gray-800">
      <page-icon :page="page" />
      <span class="ml-4">{{ page.title }}</span>
      <icon class="ml-4 text-gray-400" name="ri-eye-off-line" size="1rem" v-if="!isVisible" />
    </router-link>
      ...
      <dropdown v-on="$listeners">
        ...
        <template v-slot:content>
          <div class="flex flex-col w-48 text-gray-800">
            ...
            <confirmation-button :dropup="true" @confirm="deletePage" ref="deleteDropdown" v-if="!isIndexPage">
              <button class="flex items-center w-full px-4 py-4 hover:bg-gray-100 transition-colors duration-200 focus:outline-none">
  ...
  methods: {
    ...
    onUpdate(editedPage) {
      ...
      if (this.isCurrentPage)
        this.setCurrentPageSettings(editedPage)
      this.$emit('on-update')
    },
    clonePage() {
      this.services.page.clone(this.page.id)
      .then(() => this.$emit('on-clone'))      
    },
    showPage() {
      this.services.page.setVisible(this.page.id, true)
      .then(() => this.$emit('on-update'))
    },
    hidePage() {
      this.services.page.setVisible(this.page.id, false)
      .then(() => this.$emit('on-update'))
    },
    deletePage() {
      this.$refs.deleteDropdown.close()
      this.services.page.destroy(this.page.id)
      .then(() => {
        this.$emit('on-delete')
        if (this.isCurrentPage)
